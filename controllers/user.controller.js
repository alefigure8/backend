"use strict";

function asyncGeneratorStep(gen, resolve, reject, _next, _throw, key, arg) { try { var info = gen[key](arg); var value = info.value; } catch (error) { reject(error); return; } if (info.done) { resolve(value); } else { Promise.resolve(value).then(_next, _throw); } }

function _asyncToGenerator(fn) { return function () { var self = this, args = arguments; return new Promise(function (resolve, reject) { var gen = fn.apply(self, args); function _next(value) { asyncGeneratorStep(gen, resolve, reject, _next, _throw, "next", value); } function _throw(err) { asyncGeneratorStep(gen, resolve, reject, _next, _throw, "throw", err); } _next(undefined); }); }; }

var passport = require('passport');

var mysqlConnection = require('../database');

var registerUser = passport.authenticate('local.register', {
  successRedirect: '/user/view',
  failureRedirect: '/register'
});
var LoginUser = passport.authenticate('local.login', {
  successRedirect: '/blog/view',
  failureRedirect: '/user'
});

var listUser = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(req, res) {
    var users;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.next = 2;
            return mysqlConnection.query('SELECT * FROM users');

          case 2:
            users = _context.sent;
            res.render('./auth/view-user', {
              users: users
            });

          case 4:
          case "end":
            return _context.stop();
        }
      }
    }, _callee);
  }));

  return function listUser(_x, _x2) {
    return _ref.apply(this, arguments);
  };
}();

var deleteUser = /*#__PURE__*/function () {
  var _ref2 = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2(req, res) {
    var id;
    return regeneratorRuntime.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            id = req.params.id;
            _context2.next = 3;
            return mysqlConnection.query('DELETE FROM users WHERE id = ?', [id]);

          case 3:
            res.redirect('/user/view');

          case 4:
          case "end":
            return _context2.stop();
        }
      }
    }, _callee2);
  }));

  return function deleteUser(_x3, _x4) {
    return _ref2.apply(this, arguments);
  };
}();

module.exports = {
  registerUser: registerUser,
  LoginUser: LoginUser,
  listUser: listUser,
  deleteUser: deleteUser
};